...
AllData<- data_usr$charles

predictors<-names(AllData)
predictors<-predictors[2:53]
...
AllData_PC <- cbind(predictors_PC,classe = data_training[,54])

predictors_PC_cv<- predict(preProc,data_CV[predictors])
AllData_PC_cv   <- cbind(predictors_PC_cv,classe = data_CV[,54])
...
exam_data<- data_test2[data_test2$user_name == "charles",]
